...
import cloud.benchflow.dsl.definition.configuration._
...
trait ParameterDefinitionYamlProtocol extends DefaultYamlProtocol with ValueRangeYamlProtocol {
  ...
  implicit object MemoryDefinitionYamlFormat extends YamlFormat[MemoryDefinition] {
    ...
    override def read(yaml: YamlValue): MemoryDefinition = {
      ...
      val valueRange = yaml.asYamlObject.fields(YamlString(SystemParameterDefinition.memoryDefinitionKey)).convertTo[ValueRange[_]].asInstanceOf[ValueRange[Double]]
  ...
  implicit object CpusDefinitionYamlFormat extends YamlFormat[CpusDefinition] {
    ...
    override def read(yaml: YamlValue): CpusDefinition = {
      ...
      val valueRange = yaml.asYamlObject.fields(YamlString(SystemParameterDefinition.cpusDefinitionKey)).convertTo[ValueRange[_]].asInstanceOf[ValueRange[Double]]
