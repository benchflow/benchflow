...
public class RunBenchFlowTestTask implements Runnable {
  ...
  private static Logger logger =
      LoggerFactory.getLogger(RunBenchFlowTestTask.class.getSimpleName());
  ...
  public void run() {
    ...
    try {
      ...
      BenchFlowExperiment experiment =
          BenchFlowDSL.experimentFromTestYaml(testDefinitionYamlString);
      InputStream peDefinition =
          IOUtils.toInputStream(
              BenchFlowDSL.experimentToYamlString(experiment), StandardCharsets.UTF_8);

      // save PE defintion to minio
      minioService.saveExperimentDefinition(experimentID, peDefinition);
      ...
      bpmnModelInputStreams.forEach(
          (fileName, inputStream) ->
              minioService.copyBPMNModelForExperiment(testID, experimentID, fileName));
