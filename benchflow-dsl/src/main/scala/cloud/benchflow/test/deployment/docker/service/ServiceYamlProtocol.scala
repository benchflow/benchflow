    ...
    override def read(value: YamlValue) =  {
      ...
      fields.seq.head match {
        case (YamlString(serviceName), content) =>
          ...
          val network = params.fields.get(YamlString("network_mode"))
          match {
            case Some(YamlString(net)) => Some(Network(net))
            case _ => None
          }

          val cpuset = params.fields.get(YamlString("cpuset"))
          match {
            case Some(cpus) =>
          ...
          val memlimit = params.fields.get(YamlString("mem_limit"))
          match {
            case Some(limit) =>
