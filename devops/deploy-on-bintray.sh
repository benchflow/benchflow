...
curl -T $WERCKER_SOURCE_DIR/benchflow-dsl/target/benchflow-dsl.jar -u$BINTRAY_ADMIN_USERNAME:$BINTRAY_API_KEY "https://api.bintray.com/content/$BINTRAY_USERNAME/$BINTRAY_USERNAME-maven/$BINTRAY_USERNAME-dsl/$WERCKER_GIT_BRANCH_$CUSTOM_VERSION_TAG/benchflow-dsl_$WERCKER_GIT_BRANCH_$CUSTOM_VERSION_TAG.jar?publish=1&override=1" | jq .
echo ""
echo "Uploading artefact for branch..."
curl -T $WERCKER_SOURCE_DIR/benchflow-dsl/target/benchflow-dsl.jar -u$BINTRAY_ADMIN_USERNAME:$BINTRAY_API_KEY "https://api.bintray.com/content/$BINTRAY_USERNAME/$BINTRAY_USERNAME-maven/$BINTRAY_USERNAME-dsl/$WERCKER_GIT_BRANCH/benchflow-dsl_$WERCKER_GIT_BRANCH.jar?publish=1&override=1" | jq .