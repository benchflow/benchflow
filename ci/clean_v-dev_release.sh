...
export id=$(curl -sS -u x-oauth-basic:$GITHUB_ACCESS_TOKEN -X GET https://api.github.com/repos/benchflow/$REPO_NAME/releases/tags/$GIT_TAG | /usr/bin/jq '.id')
if [ "$id" != "null" ]; then curl -u x-oauth-basic:$GITHUB_ACCESS_TOKEN -X DELETE https://api.github.com/repos/benchflow/$REPO_NAME/releases/$id; fi
# delete tag if it already exists
git tag | if [ $(grep -c $GIT_TAG) -eq 1 ]; then git tag -d $GIT_TAG; fi
git push -q https://$GITHUB_ACCESS_TOKEN:x-oauth-basic@github.com/benchflow/$REPO_NAME :refs/tags/$GIT_TAG
# push the new tag
git tag $GIT_TAG -a -m "Development Tag"
git push -q https://$GITHUB_ACCESS_TOKEN:x-oauth-basic@github.com/benchflow/$REPO_NAME --tags