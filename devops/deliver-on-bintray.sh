...
curl -v -T $WERCKER_SOURCE_DIR/$ARTIFACT_NAME.jar -u$BINTRAY_ADMIN_USERNAME:$BINTRAY_API_KEY "https://api.bintray.com/content/${BINTRAY_USERNAME}/${BINTRAY_USERNAME}-maven/${ARTIFACT_NAME}/${WERCKER_GIT_BRANCH}/${ARTIFACT_NAME}_${WERCKER_GIT_BRANCH}_${CUSTOM_VERSION_TAG}.jar?publish=1&override=1" | jq .
echo ""
